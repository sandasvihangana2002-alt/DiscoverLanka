...
function addDays(dateString:string,amount:number){const date=new Date(`${dateString}T12:00:00`);date.setDate(date.getDate()+amount);return toDateInput(date);}
function dateOnly(value:string|null){return value?new Date(value).toISOString().slice(0,10):null;}
...
export default function PlanPage(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(typeof plan.endDate==="string")setEndDate(plan.endDate);
      ...
      const token=localStorage.getItem("discoverlanka-share-token");if(token)setShareToken(token);
    }catch{}
  },[]);

  useEffect(()=>{localStorage.setItem("discoverlanka-plan",JSON.stringify({days,travelers,interest,budget,startDate,endDate}))},[days,travelers,interest,budget,startDate,endDate]);

  const season=useMemo(()=>seasonForMonth(new Date(`${startDate||toDateInput(new Date())}T12:00:00`).getMonth()+1,seasons),[seasons,startDate]);
  const dateEvents=useMemo(()=>{
    ...
    return events.filter((event)=>{
      const eventStart=dateOnly(event.starts_at); const eventEnd=dateOnly(event.ends_at||event.starts_at);
      return Boolean(eventStart&&eventEnd&&eventStart<=to&&eventEnd>=from);
    });
  ...
  const route=useMemo(()=>{const base=(selected.length?selected:suggested.map((d)=>d.slug));const order=routeOrder[interest]??routeOrder.Mountains;return Array.from(new Set([...order.filter((slug)=>base.includes(slug)),...base.filter((slug)=>!order.includes(slug))])).map((slug)=>destinations.find((d)=>d.slug===slug)).filter(Boolean) as Destination[]},[selected,suggested,destinations,interest]);
  const routeCount=Math.max(1,route.length||suggested.length);
  ...
  const updateStart=(value:string)=>{setStartDate(value);if(value&&!endDate) setEndDate(addDays(value,days-1)); if(value&&endDate&&endDate<value)setEndDate(addDays(value,days-1)); setGenerated(false)};
  const updateEnd=(value:string)=>{if(startDate&&value<startDate)return;setEndDate(value);setGenerated(false)};
  ...
  async function generateTrip(){
    ...
    const daysPlan:ItineraryDay[]=Array.from({length:days},(_,index)=>{
      const destination=usable[index%usable.length];
      const plannedDate=startDate?addDays(startDate,index):"";
      const eventForDay=dateEvents.filter((event)=>plannedDate&&dateOnly(event.starts_at)&&dateOnly(event.starts_at)===plannedDate);
      const items=[...template];
      if(eventForDay.length)items[1]={time:"Event",title:eventForDay[0].name,detail:`${eventForDay[0].location?`${eventForDay[0].location} · `:""}${eventForDay[0].description||"Check the official source for the latest details."}`};
      return {day:index+1,destination,focus:interest,items};
    });
    ...
    setSaving(true);setSaveMessage("");setShareMessage("");
    try{
      const tripResponse=await fetch("/api/trips",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:`Sri Lanka ${days}-day ${interest} journey`,days,travelers,interest,budgetLevel:budget,budgetEstimate:estimate.total,startDate:startDate||null,endDate:endDate||null,selectedSlugs:slugs,itinerary:daysPlan,events:dateEvents})});
      const tripData=await tripResponse.json();if(!tripResponse.ok)throw new Error(tripData.error??"Trip save failed");
      const newTripId=String(tripData.trip.id);setTripId(newTripId);localStorage.setItem("discoverlanka-trip-id",newTripId);
      if(tripData.trip.share_token){const token=String(tripData.trip.share_token);setShareToken(token);localStorage.setItem("discoverlanka-share-token",token)}
      const itineraryResponse=await fetch("/api/itineraries",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tripId:newTripId,title:`${days}-day ${interest} itinerary`,summary:`Starter itinerary for ${travelers} ${travelers===1?"traveler":"travelers"}.`,durationDays:days,budgetLevel:budget,content:{days:daysPlan,startDate,endDate,events:dateEvents}})});
  ...
  async function shareTrip(){
    if(!shareToken){setShareMessage("Build the journey once to create a share link.");return;}
    const url=`${window.location.origin}/my-trip?share=${encodeURIComponent(shareToken)}`;
    try{await navigator.clipboard.writeText(url);setShareMessage("Share link copied.");}catch{setShareMessage(url)}
  }
    ...
    <section className="mx-auto max-w-7xl px-5 pb-24 pt-14 sm:px-6 md:pt-20">
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-[1fr_290px]">
        <section className="luxury-step-card p-6 sm:p-8 md:p-10">
          ...
          {step===2&&<div className="relative z-10 space-y-8 pt-7"><div><p className="text-xs font-bold uppercase tracking-[.17em] text-white/45">02 · When are you travelling?</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="rounded-2xl border border-white/8 bg-white/4 p-4"><span className="text-xs font-bold uppercase tracking-widest text-white/45">Start date</span><input type="date" value={startDate} onChange={(e)=>updateStart(e.target.value)} className="mt-3 w-full rounded-xl border border-white/10 bg-[#10251f] px-4 py-3 text-white outline-none" /></label><label className="rounded-2xl border border-white/8 bg-white/4 p-4"><span className="text-xs font-bold uppercase tracking-widest text-white/45">End date</span><input type="date" min={startDate||undefined} value={endDate} onChange={(e)=>updateEnd(e.target.value)} className="mt-3 w-full rounded-xl border border-white/10 bg-[#10251f] px-4 py-3 text-white outline-none" /></label></div>{dateEvents.length>0&&<div className="rounded-2xl border border-[#d8b875]/20 bg-[#17362b]/65 p-5"><p className="text-xs font-bold uppercase tracking-[.17em] text-[#d8b875]">Events in your window</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{dateEvents.slice(0,6).map((event)=><div key={event.id} className="rounded-xl border border-white/8 bg-black/10 p-3"><p className="font-semibold text-white/90">{event.name}</p><p className="mt-1 text-xs text-white/45">{event.location||"Sri Lanka"} · {dateOnly(event.starts_at)||"Date TBA"}</p></div>)}</div></div>}</div><div><p className="text-xs font-bold uppercase tracking-[.17em] text-white/45">02b · How long do you want to disappear for?</p><div className="mt-4 flex flex-wrap gap-2">{daysOptions.map((value)=><button key={value} onClick={()=>{setDays(value);if(startDate)setEndDate(addDays(startDate,value-1))}} className={`rounded-full px-5 py-3 text-sm font-bold ${days===value?"border border-[#d8b875]/50 bg-[#17362b] text-[#f4efe6]":"border border-white/10 bg-white/5 text-white/70"}`}>{value} days</button>)}</div></div><div><p className="text-xs font-bold uppercase tracking-[.17em] text-white/45">02c · Who is travelling?</p><div className="mt-4 flex items-center gap-3"><button onClick={()=>setTravelers(Math.max(1,travelers-1))} className="luxury-step-number h-11 w-11">−</button><span className="w-14 text-center text-2xl font-semibold">{travelers}</span><button onClick={()=>setTravelers(Math.min(10,travelers+1))} className="luxury-step-number h-11 w-11">+</button><span className="text-sm text-white/45">traveller{travelers===1?"":"s"}</span></div></div><div className="flex flex-wrap gap-3"><button onClick={back} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/70">← Back</button><button onClick={next} className="rounded-full bg-[#d9a441] px-6 py-3 text-sm font-bold text-[#10251f]">Continue →</button></div></div>}

          {step===3&&<div className="relative z-10 pt-7"><div><p className="text-xs font-bold uppercase tracking-[.17em] text-white/45">03 · Choose your travel style</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{budgets.map((item)=><button key={item} onClick={()=>setBudget(item)} className={`rounded-2xl border p-5 text-left ${budget===item?"border-[#d8b875]/55 bg-[#17362b]":"border-white/8 bg-white/4"}`}><p className="text-lg font-semibold">{item}</p><p className="mt-1 text-xs text-white/45">{money(dailyRates[item])} base / person / day</p></button>)}</div></div><div className="mt-8"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.17em] text-white/45">03b · Pick places to carry with you</p><p className="mt-1 text-sm text-white/45">{selected.length} selected · the route order adapts to your interest.</p></div><button onClick={()=>setSelected([])} className="text-xs font-bold uppercase tracking-[.15em] text-[#d8b875]">Clear</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{routeCandidates.map((destination)=><button key={destination.slug} onClick={()=>toggleDestination(destination.slug)} className={`group relative min-h-[175px] overflow-hidden rounded-2xl border text-left ${selected.includes(destination.slug)?"border-[#d8b875]/60":"border-white/8"}`}><img src={destination.image} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-[#07120f] via-[#07120f]/45 to-transparent"/><div className="relative flex h-full items-end justify-between p-4"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#d8b875]">{destination.region}</p><p className="mt-1 text-lg font-semibold">{destination.name}</p></div><span className={`rounded-full border px-3 py-1 text-[10px] font-bold ${selected.includes(destination.slug)?"border-[#d8b875]/40 bg-[#d8b875]/16 text-[#f4efe6]":"border-white/15 bg-black/20 text-white/65"}`}>{selected.includes(destination.slug)?"Selected":"+ Add"}</span></div></button>)}</div></div>{dateEvents.length>0&&<div className="mt-7 rounded-2xl border border-[#d8b875]/20 bg-[#17362b]/55 p-5"><p className="text-xs font-bold uppercase tracking-[.17em] text-[#d8b875]">Route-aware events</p><p className="mt-2 text-sm text-white/55">{dateEvents.length} event{dateEvents.length===1?"":"s"} fall inside your travel window and will be considered in the itinerary.</p></div>}<div className="mt-8 flex flex-wrap gap-3"><button onClick={back} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/70">← Back</button><button onClick={generateTrip} disabled={loading} className="rounded-full bg-[#d9a441] px-6 py-3 text-sm font-bold text-[#10251f] disabled:opacity-50">Build my journey ↗</button></div></div>}

          {step===4&&<div className="relative z-10 pt-7"><div className="grid gap-4 md:grid-cols-3"><div className="rounded-2xl border border-white/8 bg-white/4 p-5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-white/45">Journey</p><p className="mt-2 text-2xl font-semibold">{days} days · {travelers} traveller{travelers===1?"":"s"}</p><p className="mt-1 text-sm text-white/45">{interest} · {budget}</p></div><div className="rounded-2xl border border-white/8 bg-white/4 p-5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-white/45">Dates</p><p className="mt-2 text-lg font-semibold">{startDate&&endDate?`${startDate} → ${endDate}`:"Flexible dates"}</p><p className="mt-1 text-sm text-white/45">{dateEvents.length} event{dateEvents.length===1?"":"s"} in window</p></div><div className="rounded-2xl border border-[#d8b875]/20 bg-[#17362b]/60 p-5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#d8b875]">Estimated total</p><p className="mt-2 text-2xl font-semibold">{money(estimate.total)}</p><p className="mt-1 text-sm text-white/45">Planning estimate, not a booking quote.</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{route.map((destination,index)=><div key={destination.slug} className="golden-route-node rounded-2xl border border-white/8 bg-black/10 p-4 pl-10"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#d8b875]">Stop {index+1}</p><p className="mt-1 font-semibold">{destination.name}</p><p className="mt-0.5 text-xs text-white/45">{destination.region}</p></div>)}</div><div className="mt-6 flex flex-wrap gap-3"><button onClick={()=>setStep(3)} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/70">← Adjust</button><button onClick={generateTrip} className="rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/80">↻ Rebuild</button><a href="/my-trip" className="rounded-full bg-[#d9a441] px-6 py-3 text-sm font-bold text-[#10251f]">Open My Trip →</a><button onClick={shareTrip} className="rounded-full border border-[#d8b875]/30 bg-[#d8b875]/10 px-6 py-3 text-sm font-bold text-[#d8b875]">Share trip ↗</button></div>{saveMessage&&<p className="mt-4 text-sm text-[#d8b875]">{saveMessage}{saving?" Saving…":""}</p>}{shareMessage&&<p className="mt-2 text-sm text-[#d8b875]">{shareMessage}</p>}<p className="mt-4 text-xs text-white/35">Trip ID: {tripId??"Browser draft"}</p></div>}
        </section>

        <aside className="luxury-step-card h-fit p-6 sm:p-7"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#d8b875]">Your desk</p><div className="mt-4 space-y-3"><div className="rounded-2xl border border-white/8 bg-white/4 p-4"><p className="text-[10px] uppercase tracking-[.16em] text-white/45">Feeling</p><p className="mt-1 font-semibold">{interest}</p></div><div className="rounded-2xl border border-white/8 bg-white/4 p-4"><p className="text-[10px] uppercase tracking-[.16em] text-white/45">Dates</p><p className="mt-1 font-semibold">{startDate&&endDate?`${startDate} → ${endDate}`:`${days} days`}</p></div><div className="rounded-2xl border border-white/8 bg-white/4 p-4"><p className="text-[10px] uppercase tracking-[.16em] text-white/45">Style</p><p className="mt-1 font-semibold">{budget}</p></div></div><div className="mt-5 border-t border-white/8 pt-5"><p className="text-[10px] uppercase tracking-[.16em] text-white/45">Estimated</p><p className="mt-1 text-2xl font-semibold">{money(estimate.total)}</p><p className="mt-2 text-xs leading-5 text-white/38">Accommodation {money(estimate.accommodation)} · Food {money(estimate.food)} · Transport {money(estimate.transport)} · Activities {money(estimate.activities)}</p></div></aside>
      </div>

      {generated&&<section id="itinerary" className="luxury-step-card mt-10 p-7 sm:p-9"><div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#d8b875]">Live itinerary</p><h2 className="mt-2 text-3xl font-semibold">A first draft worth refining.</h2><p className="mt-2 text-sm text-white/50">{days} days · {interest} · {budget}{startDate?` · ${startDate} → ${endDate||addDays(startDate,days-1)}`:""}</p></div><div className="flex gap-3"><a href="/my-trip" className="text-sm font-bold text-[#d8b875]">Open My Trip →</a><button onClick={shareTrip} className="text-sm font-bold text-[#d8b875]">Share ↗</button></div></div><div className="mt-7 grid gap-4 lg:grid-cols-2">{itinerary.map((day)=><article key={day.day} className="rounded-2xl border border-white/8 bg-black/10 p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#d8b875]">Day {day.day}</p><h3 className="mt-1 text-xl font-semibold">{day.destination.name}</h3></div><span className="text-xs text-white/45">{day.destination.region}</span></div><div className="mt-4 grid gap-2 sm:grid-cols-3">{day.items.map((item,index)=><div key={index} className="rounded-xl border border-white/7 bg-white/4 p-3"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#d8b875]">{item.time}</p><p className="mt-1 text-sm font-semibold text-white/88">{item.title}</p></div>)}</div></article>)}</div>{dateEvents.length>0&&<div className="mt-5 rounded-2xl border border-[#d8b875]/15 bg-[#17362b]/45 p-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#d8b875]">Events considered</p><div className="mt-3 flex flex-wrap gap-2">{dateEvents.map((event)=><span key={event.id} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-white/60">{event.name}</span>)}</div></div>}</section>}
