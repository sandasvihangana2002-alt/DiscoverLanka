const destinations = [
  { name: "Kandy", type: "Culture & hills", image: "https://images.unsplash.com/photo-1548013146-72479768bada?auto=format&fit=crop&w=1200&q=80" },
  { name: "Ella", type: "Mountains & trains", image: "https://images.unsplash.com/photo-1586613837427-44f7a0b1e5e6?auto=format&fit=crop&w=1200&q=80" },
  { name: "Galle", type: "Coast & heritage", image: "https://images.unsplash.com/photo-1566296314736-6eaac1ca0cb9?auto=format&fit=crop&w=1200&q=80" },
];
...
export default function Home() {
  return (
    <main>
      ...
      <section id="top" className="hero-grid relative flex min-h-[760px] items-end overflow-hidden bg-[#183d32] text-white">
      ...
      <section id="discover" className="mx-auto max-w-7xl px-6 py-24">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-[#b27c1d]">Where to next?</p><h2 className="mt-3 text-4xl font-semibold tracking-tight md:text-5xl">Start with a place.</h2></div><p className="max-w-md text-[#66756f]">Handpicked destinations for every kind of traveller — with practical details to help you decide.</p></div>
        <div className="mt-10 grid gap-5 md:grid-cols-3">{destinations.map((d) => <article key={d.name} className="group relative h-[420px] overflow-hidden rounded-3xl bg-[#183d32] text-white"><img src={d.image} alt={d.name} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent"/><div className="absolute bottom-0 p-7"><p className="text-sm text-white/70">{d.type}</p><h3 className="mt-1 text-3xl font-semibold">{d.name}</h3></div></article>)}</div>
      </section>
      ...
      <section id="stories" className="border-t border-black/5 px-6 py-24"><div className="mx-auto max-w-7xl"><div className="flex items-end justify-between"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-[#b27c1d]">Stories from the island</p><h2 className="mt-3 text-4xl font-semibold tracking-tight md:text-5xl">Go beyond the guidebook.</h2></div><span className="hidden text-sm text-[#66756f] md:block">Local voices · Real journeys</span></div></div></section>
