...
const experienceData = [
  { title: "Wild Sri Lanka", text: "Safaris, forests and close encounters with the island's wild side.", icon: "◒" },
  { title: "Food & flavour", text: "Markets, local kitchens and the dishes that tell Sri Lankan stories.", icon: "◌" },
  { title: "Culture & heritage", text: "Temples, ancient cities, crafts and living traditions.", icon: "◈" },
  { title: "Slow escapes", text: "Tea country, quiet beaches and days with nowhere to rush.", icon: "⌁" },
];
  ...
  return (
    <main>
      ...
      <section id="discover" className="mx-auto max-w-7xl px-6 py-24">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-[#b27c1d]">Where to next?</p><h2 className="mt-3 text-4xl font-semibold tracking-tight md:text-5xl">Start with a place.</h2></div><p className="max-w-md text-[#66756f]">Save places as you explore, then use them as the starting point for your trip.</p></div>
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {destinations.slice(0, 6).map((d) => (
            <div key={d.id} className="group relative h-[420px] overflow-hidden rounded-3xl bg-[#183d32] text-white">
              <a href={`/destinations/${d.slug}`} className="absolute inset-0" aria-label={`Explore ${d.name}`}><img src={destinationImages[d.slug] ?? destinationImages.kandy} alt={d.name} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/15 to-transparent" /></a>
              <button onClick={() => toggleSaved(d.slug)} aria-label={saved.includes(d.slug) ? `Remove ${d.name} from My Trip` : `Save ${d.name} to My Trip`} className="absolute right-5 top-5 z-10 rounded-full bg-white/90 px-4 py-2 text-sm font-bold text-[#183d32] shadow-sm">{saved.includes(d.slug) ? "♥ Saved" : "♡ Save"}</button>
              <div className="absolute bottom-0 z-10 w-full p-7"><p className="text-sm text-white/70">{d.region}</p><h3 className="mt-1 text-3xl font-semibold">{d.name}</h3><p className="mt-2 max-w-sm text-sm text-white/70">{d.summary}</p><a href={`/destinations/${d.slug}`} className="mt-4 inline-block text-sm font-bold text-[#e7c36e]">Explore destination →</a></div>
            </div>
          ))}
      ...
      <section id="experiences" className="bg-[#183d32] px-6 py-24 text-white"><div className="mx-auto max-w-7xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-[#e7c36e]">More than places</p><h2 className="mt-3 max-w-3xl text-4xl font-semibold tracking-tight md:text-6xl">Travel for the feeling, not just the photo.</h2><div className="mt-12 grid gap-4 md:grid-cols-4">{experienceData.map((x)=><a href={`/plan?interest=${encodeURIComponent(x.title.split(" ")[0])}`} key={x.title} className="rounded-2xl border border-white/15 bg-white/5 p-6 transition hover:-translate-y-1 hover:bg-white/10"><div className="mb-16 text-4xl">{x.icon}</div><h3 className="text-xl font-semibold">{x.title}</h3><p className="mt-2 text-sm leading-6 text-white/60">{x.text}</p><span className="mt-5 inline-block text-sm font-bold text-[#e7c36e]">Plan around this →</span></a>)}</div></div></section>

      <section id="map" className="border-y border-black/5 bg-[#f1eee6] px-6 py-24"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-[#b27c1d]">Explore the island</p><h2 className="mt-3 text-4xl font-semibold tracking-tight md:text-5xl">Find your next stop on the map.</h2></div><a href="https://www.openstreetmap.org/?mlat=7.8731&mlon=80.7718#map=8/7.8731/80.7718" target="_blank" rel="noreferrer" className="font-bold text-[#183d32]">Open full map ↗</a></div><div className="mt-10 overflow-hidden rounded-[2rem] border border-black/10 bg-white"><iframe title="Sri Lanka map" src="https://www.openstreetmap.org/export/embed.html?bbox=79.6%2C5.8%2C82.1%2C9.9&layer=mapnik" className="h-[430px] w-full border-0" loading="lazy" /><div className="grid gap-3 p-5 sm:grid-cols-2 md:grid-cols-4">{destinations.slice(0, 8).map((d)=><a key={d.id} href={`/destinations/${d.slug}`} className="rounded-xl bg-[#f7f5ef] px-4 py-3 text-sm font-semibold hover:bg-[#e9e2d3]"><span>{d.name}</span><span className="ml-2 text-[#8d651d]">→</span></a>)}</div></div></div></section>

      <section id="plan" className="px-6 py-24"><div className="mx-auto max-w-5xl rounded-[2rem] bg-[#e9e2d3] p-8 md:p-14"><p className="text-sm font-bold uppercase tracking-[.2em] text-[#8d651d]">Your journey starts here</p><h2 className="mt-4 text-4xl font-semibold tracking-tight md:text-6xl">Where should you go?</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-[#66756f]">Tell us how you want to travel. We’ll help turn your interests, time and budget into a Sri Lankan journey worth remembering.</p><div className="mt-9 grid gap-3 sm:grid-cols-3"><a href="/plan?interest=Mountains" className="rounded-2xl bg-white p-5 text-left font-semibold shadow-sm">🏔️ Mountains<br/><span className="text-sm font-normal text-[#66756f]">Cool air & adventure</span></a><a href="/plan?interest=Coast" className="rounded-2xl bg-white p-5 text-left font-semibold shadow-sm">🌊 Coast<br/><span className="text-sm font-normal text-[#66756f]">Beaches & sunsets</span></a><a href="/plan?interest=Wildlife" className="rounded-2xl bg-white p-5 text-left font-semibold shadow-sm">🌿 Wild<br/><span className="text-sm font-normal text-[#66756f]">Nature & wildlife</span></a></div><a href="/plan" className="mt-5 inline-block rounded-full bg-[#183d32] px-7 py-3.5 font-bold text-white">Build my trip →</a></div></section>
