...
export default function Home() {
  ...
  return (
    <main className="home-rebuild min-h-screen bg-[#f4efe5] text-[#17251f]">
      ...
      <section id="experiences" className="home-kandyan-experiences relative overflow-hidden bg-[#0a1712] px-6 py-20 text-white sm:px-8 lg:px-10 lg:py-24"><img src="https://d104tpg70nvstz.cloudfront.net/2016/09/cultural-feat.jpg" alt="Traditional Kandyan dance performance in Sri Lanka" className="absolute inset-0 h-full w-full object-cover object-center opacity-55"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,13,10,.96)_0%,rgba(5,13,10,.80)_38%,rgba(5,13,10,.48)_100%)]"/><div className="absolute inset-0 opacity-40 bg-[radial-gradient(circle_at_78%_20%,rgba(216,184,117,.18),transparent_24%),radial-gradient(circle_at_30%_90%,rgba(107,33,33,.16),transparent_30%)]"/><div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[300px_1fr] lg:items-center"><div><div className="home-kandyan-kicker"><span>◆</span><p>Explore by experience</p><span>◆</span></div><h2 className="mt-4 font-serif text-4xl leading-[.98] tracking-[-.035em] sm:text-5xl">Find your<br/><span>kind of Sri Lanka.</span></h2><p className="mt-5 text-sm leading-7 text-white/65">From living heritage to wild landscapes and slow island days, choose the experience that feels like you.</p><a href="/experiences" className="home-kandyan-link mt-6 inline-flex items-center gap-3 text-[10px] font-black uppercase tracking-[.18em]">Explore experiences <span>↗</span></a></div><div className="home-kandyan-grid grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{categories.map((name, i) => <a key={name} href={`/experiences?category=${encodeURIComponent(name)}`} className="home-kandyan-card group"><div className="home-kandyan-card-frame"><span className="home-kandyan-card-corner home-kandyan-card-corner-tl"/><span className="home-kandyan-card-corner home-kandyan-card-corner-br"/><span className="home-kandyan-number">0{i + 1}</span><div className="home-kandyan-icon">{experienceIcons[name] ?? "✦"}</div><div className="home-kandyan-name">{name}</div><div className="home-kandyan-line"/><span className="home-kandyan-arrow">↗</span></div></a>)}</div></div></section>
