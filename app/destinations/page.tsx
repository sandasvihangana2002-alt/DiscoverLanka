          ...
          {destinations.map((destination,index)=>{
            ...
            return <a key={destination.id} href={`/destinations/${destination.slug}`} className="destination-travel-card group">
              <div className="destination-card-image-wrap">
                <img src={image} alt={destination.name} className="destination-card-image" loading={index<4?"eager":"lazy"} />
                <div className="destination-card-image-shade"/>
                <div className="destination-card-top">
                  <span>{destination.region}</span><span className="destination-heart" aria-hidden="true">♡</span>
                </div>
                <div className="destination-card-number">{String(index+1).padStart(2,"0")}</div>
              </div>
              <div className="destination-card-body">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><h2 className="destination-card-title">{destination.name}</h2><p className="destination-card-copy">{destination.summary}</p></div>
                  <span className="destination-card-arrow" aria-hidden="true">↗</span>
                </div>
                <div className="destination-card-meta-row">
                  <span className="destination-rating"><b>★</b>{destination.review_count>0?`${destination.review_count} reviews`:"No reviews yet"}</span>
