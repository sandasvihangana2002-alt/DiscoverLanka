...
export default async function DestinationsPage(){
  ...
  return (
    <main className="destinations-page min-h-screen bg-[#07120f] text-[#f4efe6]">
      <header className="home-nav fixed inset-x-0 top-0 z-50 px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-[#d9b972]/25 bg-[#07140f]/52 px-5 py-3.5 text-white shadow-[0_18px_60px_rgba(0,0,0,.24)] backdrop-blur-2xl sm:px-7">
          ...
          <nav className="hidden items-center gap-8 text-[10px] font-black uppercase tracking-[.17em] text-white/70 lg:flex" aria-label="Primary navigation">
            <a className="home-nav-link" href="/">Discover</a><a className="home-nav-link" href="/destinations">Destinations</a><a className="home-nav-link" href="/stories">Stories</a><a className="home-nav-link" href="/plan">Plan</a><a className="home-nav-link" href="/#map">Map</a><a className="home-nav-link" href="/my-trip">My Trip</a>
          </nav>
      ...
      <style>{`
.destination-card-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px;align-items:start}
...
.destination-card-image-wrap{position:relative;height:380px;overflow:hidden;background:#16382f}
...
.destination-card-body{position:relative;min-height:174px;margin:-24px 14px 14px;padding:25px 22px 20px;border-radius:22px;background:linear-gradient(145deg,#fbf8f1,#eee7d9);box-shadow:0 12px 30px rgba(0,0,0,.12);z-index:2}
.destination-card-title{margin:0;color:#10251f !important;font-family:"amoera","Amoera",sans-serif !important;font-size:clamp(1.75rem,1.48rem + .62vw,2.3rem);font-weight:700;letter-spacing:-.018em;line-height:1.02;overflow-wrap:anywhere}
...
@media(max-width:1100px){
  .destination-card-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}
}
@media(max-width:820px){
  .destination-card-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}
  .destination-card-image-wrap{height:340px}
}
